...
def process_embedding_file(input_file, model_name):
    ...
    plt.title(f"Trend of Alien Belief Over Time ({model_name})\n"
              "(0 = Aliens do not exist, 100 = Aliens do exist)")
    plt.xlabel("Year")
    ...
    output_file = f"plots/{model_name}_alien_claims_ngram_market.png"
...
def main():
    ...
    embedding_files = glob.glob("*_alien_claims_embedded.jsonl")
    ...
    for embedding_file in embedding_files:
        ...
        # e.g., "openai_alien_claims_embedded.jsonl" -> "openai"
        if embedding_file == "alien_claims_embedded.jsonl":
            model_name = "default"
        else:
            model_name = embedding_file.replace("_alien_claims_embedded.jsonl", "")
